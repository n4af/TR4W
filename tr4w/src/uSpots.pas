...
type
  ...
  protected
    ...
    procedure InsertSpotBuffer(Index: integer; const Spot: TSpotRecord);
      virtual;
    // Gav 4.45.6
...
begin
  ...
  if Spot.FBand in [Band60, Band30, Band17, Band12] then
    Spot.FWARCBand := True;
  if (IE_Switch) then
  begin
    ie_check := True;
...
begin
 ...
 //    Exit; // Gav 4.45.6
  ...
  for i := 0 to FCount - 1 do
  begin
   if (Flist^[i].FCall = Flist^[i+1].FCall) and (i < FCount-1) then
    delete(i);
    if Flist^[i].FFrequency = 0  then
     delete(i);
    if not BandMapAllBands then
      if FList^[i].FBand <> BandmapBand then
        Continue; //Gav  ActiveBand changed to BandmapBand
...
procedure TDXSpotsList.InsertSpotBuffer(Index: integer; const Spot:
  TSpotRecord);
// Gav 4.45.6
begin
