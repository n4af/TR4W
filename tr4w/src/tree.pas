...
//  if Windows.FindFirstFile(PChar(FileName), find_data) <> INVALID_HANDLE_VALUE then
  ...
  if FileExistsUTF8(@FileName[1]) { *Converted from FileExists* } then
  begin
...
begin
  ...
  Windows.DeleteFileUTF8(PChar(NewName)); { *Converted from DeleteFile* }
...
begin
  ...
  Result :=
    FileCreate(wsprintfBuffer); { *Converted from CreateFile* }
end;
