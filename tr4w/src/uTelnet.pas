...
//  ShowMessage(@TelnetBuffer[DX + 24]);
  if TelnetBuffer[DX + 24] = '.' then Offset := 2 else Offset := 0; // 4.92.6
  if TelnetBuffer[DX + 26] = '.' then Offset := 4 else Offset := 0;
  if TelnetBuffer[Dx + 23] = '.' then Offset := 1 else Offset := 0;
  if TelnetBuffer[Dx + 21] = '.' then Offset := -1 else Offset := 0;
  {Source Callsign}
  for i := DX + 9 to DX + 20 do
  begin
    if TelnetBuffer[i] = ':' then
    begin
      if TelnetBuffer[i + 1] <> ' ' then Offset :=  1 else Offset := 0;      // 4.92.6
