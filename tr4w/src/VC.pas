...
type
  ...
  SCPIndexArrayType = array[0..36, 0..36] of longword {LONGINT n6tr}
    {LongWord {ua4wli};
...
const
  ...
  MAINTR4WDLGTEMPLATE: TR4WDLGTEMPLATE =
    (Style: DS_MODALFRAME or DS_3DLOOK or WS_POPUP or WS_CAPTION or WS_SYSMENU or
      WS_THICKFRAME; dwExtendedStyle: WS_EX_DLGMODALFRAME or WS_EX_TOOLWINDOW);
...
{$IFEND}
  ...
  OZCR2008 = False;
  LOGVERSION = 'v1.6';
    // This is broken out below for comparison later on. Again, needlessly complex. de NY4I
  LOGVERSION1 = 'v';
  LOGVERSION2 = '1';
  LOGVERSION3 = '.';
  LOGVERSION4 = '6'; // ny4i Added ExtendedMode to ContestExchange
  CURRENTVERSIONASINTEGER = Ord(LOGVERSION1) + Ord(LOGVERSION2) * 256 +
    Ord(LOGVERSION3) * $10000 + Ord(LOGVERSION4) * $1000000;
  TR4W_DOWNLOAD_LINK: PChar = 'http://www.tr4w.net/download/?' +
...
const
  ...
  DefStyleNoSun = WS_CHILD or SS_NOTIFY or SS_center or SS_NOPREFIX or
    WS_VISIBLE;
  ...
  LeftStyle = WS_CHILD or SS_NOTIFY or SS_LEFT or SS_NOPREFIX or SS_SUNKEN or
    WS_VISIBLE;
  RightStyle = WS_CHILD or SS_NOTIFY or SS_RIGHT or SS_NOPREFIX or SS_SUNKEN or
    WS_VISIBLE;
  LeftVisNoSunStyle = WS_CHILD or SS_NOTIFY or SS_LEFT or SS_NOPREFIX or
    WS_VISIBLE;
...
type
  ...
  RemainingMultiplierType = (rmNoRemMultDisplay, rmDomestic, rmDX, rmZone,
    rmPrefix);
...
const
  CabrilloExtantionSA: array[CabrilloExtantionType] of PChar = ('%s%s.CBR',
    '%s%s.LOG');
...
type
  ...
  tCategoryBand = (cbALL, cb160M, cb80M, cb40M, cb20M, cb15M, cb10M, cb6M, cb2M,
    cb222, cb432, cb902, cb12G);
...
const
  // Use uppercase
  tLogLevelsSA: array[tLogLevels] of PChar = ('NONE', 'FATAL', 'ERROR', 'WARN',
    'INFO', 'DEBUG', 'TRACE');
  tCategoryAssistedSA: array[tCategoryAssisted] of PChar = ('NON-ASSISTED',
    'ASSISTED');
  tCategoryBandSA: array[tCategoryBand] of PChar = ('ALL', '160M', '80M', '40M',
    '20M', '15M', '10M', '6M', '2M', '222', '432', '902', '1.2G');
  tCategoryModeSA: array[tCategoryMode] of PChar = ('CW', 'RTTY', 'DIGI', 'SSB',
    'MIXED'); // 4.90.14
  tCertificateSA: array[tCertificate] of PChar = ('Yes', 'No');
  tCategoryOperatorSA: array[tCategoryOperator] of PChar = ('SINGLE-OP',
    'MULTI-OP', 'CHECKLOG');
  tCategoryPowerSA: array[tCategoryPower] of PChar = ('HIGH', 'LOW', 'QRP');
  tCategoryTransmitterSA: array[tCategoryTransmitter] of PChar = ('ONE', 'TWO',
    'LIMITED', 'UNLIMITED', 'SWL');
...
type
  ...
  CFGFunc = (cfAll, {cfR1, cfR2, cfHard, cfBM, }cfCol, cfAppearance, cfWK,
    cfRadio1, cfRadio2);

  CFGStatus = (csNew, csOld, csRem);

  CFGType = (ctFreqList, ctDirectory, ctFileName, ctMessage, ctMultiplier,
    ctBoolean, ctReal, ctByte, ctInteger, ctWord, ctString, ctURL, ctOperation,
    ctOther, ctChar, ctAlphaChar, {ctPort, }ctPortLPT, ctBand);
...
type
  ...
  ExtendedModeType = (eNoMode, eCW, eRTTY, eFT8, eFT4, eJT65, ePSK31, ePSK63,
    eSSB, eFM, eAM,
    eMFSK, eJS8, eUSB, eLSB, eData, eCW_R, eData_R, eRTTY_R, eFM_N, eAM_N,
      eData_FM, eC4FM, eDStar, ePSK_R, eWFM);

  ModeType = (CW, Digital, Phone, Both, NoMode, FM); { Use for TR }
  //AM, CW, CW-R, DATA-L, DATA-U, FM, LSB, USB, RTTY, RTTY-R, WBFM
  ExtendedRadioModeType = (rNoMode, rCW, rLSB, rUSB, rAM, rFM, rRTTY, rRTTY_R,
    rAFSK, rDATA_U, rDATA_L, rDIGITAL, rCW_R);
...
const
  ...
  MonthTags: array[1..12] of PChar = ('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun',
    'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec');

  CallstringLength = 13;

  ADIFModeString: array[ModeType] of PChar = ('CW', 'DIGITAL', 'SSB', 'BTH',
    'NON', 'FM');
  ModeStringArray: array[ModeType] of PChar = ('CW', 'DIGI', 'SSB', 'BTH', 'NON',
    'FM');

  ExtendedModeStringArray: array[ExtendedModeType] of string =
    ('NoMode', 'CW', 'RTTY', 'FT8', 'FT4', 'JT65', 'PSK31', 'PSK63', 'SSB', 'FM',
      'AM',
    'MFSK', 'JS8', 'USB', 'LSB', 'DATA', 'CW-R', 'DATA-R', 'RTTY-R', 'FM-N',
      'AM-N',
...
type
  StationStatusType = (sstComputerNameAndID, sstBandModeFreq, sstPTT, sstOpMode,
    sstQSOs, sstCallsign);
...
type
  ...
  UpadateAction = (actSetClearDupesheetBit, actResetClearDupesheetBit,
    actRescore, actGetCRC32, actClearMults);
...
type
  ...
  ContestExchangev1_5 = record
    ...
    {11}DomesticQTH: Str10;
      //Corrected QTH - if it is need  - i.e. AF1 -> AF-001 in IOTA contest.
    {01}ZERO_07: DummyByte;
    ...
    {02}RSTSent: smallInt; {Word;}
    {02}RSTReceived: smallInt; {Word;}
      // if this was an int, I could put ft8 reports here.
...
const
  CallsignExchangeWinStyle = WS_CHILD or WS_VISIBLE or ES_UPPERCASE or WS_TABSTOP
    or ES_AUTOHSCROLL {or ES_OEMCONVERT  } or ES_NOHIDESEL;

  QSOMULTSWINDOWSTYLE = uVisStyleNoSun;
  QSOMULTSMODEWINDOWSTYLE = WS_CHILD or SS_NOTIFY or SS_RIGHT or SS_NOPREFIX or
    WS_VISIBLE;
type
...
{$IFEND}
  ...
  tr4w_WinClass: TWndClass = (Style: CS_DBLCLKS; hbrBackground: COLOR_BTNFACE +
    1; {lpszMenuName: 'T'; }lpszClassName: tr4w_ClassName; );
...
{*)}
    ...
    const
      ContestsArray: array[ContestType] of TContestInfo =
        ...
        (Name: 'DUMMY CONTEST'; Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE: UnknownExchange;
          XM: NoDXMults; QP: NoQSOPointMethod; ADIFName: ''; CABName: ''),
        (Name: '7QP'; Email: nil; DF: 'seven_cty'; WA7BNM: 404;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: DomesticFile; P: 10; AE:
          RSTDomesticOrDXQTHExchange; XM: NoDXMults; QP: TwoPhoneThreeCW; ADIFName:
          '7QP'; CABName: ''),
        (Name: 'ALL ASIAN CW'; Email: nil; DF: nil; WA7BNM: 47;
          {SK3BG: 'allasia';    }QRZRUID: 146; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTAgeExchange; XM: NoDXMults; QP: AllAsianQSOPointMethod; ADIFName:
          'ALL-ASIAN-DX-CW'; CABName: ''),
        (Name: 'ALL ASIAN SSB'; Email: nil; DF: nil; WA7BNM: 102;
          {SK3BG: 'allasia';    }QRZRUID: 146; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTAgeExchange; XM: NoDXMults; QP: AllAsianQSOPointMethod; ADIFName:
          'AL-ASIAN-DX-PHONE'; CABName: ''),
        ({Name: 'ALL JA';                     }Email: nil; DF: 'allja'; WA7BNM:
          0000; {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTALLJAPrefectureAndPrecedenceExchange; XM: NoDXMults; QP:
          OnePointPerQSO; ADIFName: ''; CABName: ''),
        ({Name: 'AP-SPRINT';                  }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: 'asiapacs';   }QRZRUID: 77; Pxm: Prefix; ZnM: NoZoneMults; AIE:
          NoInitialExchange; DM: NoDomesticMults; P: 0; AE: RSTQSONumberExchange;
          XM: NoDXMults; QP: OnePointPerQSO; ADIFName: ''; CABName: ''),
        ({Name: 'ARCI';                       }Email: nil; DF: 's50p12'; WA7BNM:
          0000; {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTPossibleDomesticQTHAndPower; XM: ARRLDXCCWithNoUSACanadaKH6OrKL7; QP:
          ARCIQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'ARI-DX';                     }Email: nil; DF: 'ari'; WA7BNM: 9;
          {SK3BG: nil;          }QRZRUID: 85; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTAndQSONumberOrDomesticQTHExchange; XM: ARRLDXCCWithNoIOrIS0; QP:
          ARIQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'ARRL-10';                    }Email: '10meter@arrl.org'; DF:
          'arrl10'; WA7BNM: 199; {SK3BG: 'arrl10';     }QRZRUID: 14; Pxm:
          NoPrefixMults; ZnM: NoZoneMults; AIE: NoInitialExchange; DM:
          DomesticFile; P: 0; AE: RSTDomesticQTHOrQSONumberExchange; XM:
          ARRLDXCCWithNoUSACanadaKH6OrKL7; QP: ARRL10QSOPointMethod; ADIFName: '';
          CABName: ''),
        ({Name: 'ARRL DIGI';                  }Email: 'contests@arrl.org'; DF:
          nil; WA7BNM: 716; QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults; AIE:
          GridInitialExchange; DM: NoDomesticMults {GridFields}; P: 0; AE:
          Grid2Exchange; XM: NoDXMults; QP: ARRLDIGIQSOPointMethod; ADIFName: '';
          CABName: 'ARRL-DIGI'), // Issue 577
        ({Name: 'ARRL-160';                   }Email: '160meter@arrl.org'; DF:
          'arrlsect'; WA7BNM: 194; {SK3BG: 'arrl160';    }QRZRUID: 22; Pxm:
          NoPrefixMults; ZnM: NoZoneMults; AIE: NoInitialExchange; DM:
          DomesticFile; P: 0; AE: RSTDomesticOrDXQTHExchange; XM: NoDXMults; QP:
          ARRL160QSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'ARRL-DX-CW';                 }Email: 'DXCW@arrl.org'; DF: nil;
          WA7BNM: 256; {SK3BG: 'arrlidx';    }QRZRUID: 335; Pxm: NoPrefixMults;
          ZnM: NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTPowerExchange; XM: NoDXMults; QP: ARRLDXQSOPointMethod; ADIFName: '';
          CABName: ''),
        ({Name: 'ARRL-DX-SSB';                }Email: 'DXPhone@arrl.org'; DF:
          nil; WA7BNM: 289; {SK3BG: 'arrlidx';    }QRZRUID: 334; Pxm:
          NoPrefixMults; ZnM: NoZoneMults; AIE: NoInitialExchange; DM:
          NoDomesticMults; P: 0; AE: RSTPowerExchange; XM: NoDXMults; QP:
          ARRLDXQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'ARRL-SS-CW';                 }Email: 'SSCW@arrl.org'; DF:
          'arrlsect'; WA7BNM: 177; {SK3BG: 'arrlnoss';   }QRZRUID: 0; Pxm:
          NoPrefixMults; ZnM: NoZoneMults; AIE: CheckSectionInitialExchange; DM:
          DomesticFile; P: 0; AE: QSONumberPrecedenceCheckDomesticQTHExchange; XM:
          NoDXMults; QP: TwoPointsPerQSO; ADIFName: ''; CABName: ''),
        ({Name: 'ARRL-SS-SSB';                }Email: 'SSPhone@arrl.org'; DF:
          'arrlsect'; WA7BNM: 178; {SK3BG: 'arrlnoss';   }QRZRUID: 0; Pxm:
          NoPrefixMults; ZnM: NoZoneMults; AIE: CheckSectionInitialExchange; DM:
          DomesticFile; P: 0; AE: QSONumberPrecedenceCheckDomesticQTHExchange; XM:
          NoDXMults; QP: TwoPointsPerQSO; ADIFName: ''; CABName: ''),
        ({Name: 'ARRL VHF JAN';              }Email: nil; DF: nil; WA7BNM: 43;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: GridSquares; P: 0; AE: GridExchange; XM:
          NoDXMults; QP: ARRLVHFJUNPointMethod; ADIFName: 'ARRL-VHF-JAN'; CABName:
          ''),
        ({Name: 'ARRL VHF JUN';               }Email: nil; DF: nil; WA7BNM: 43;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: GridSquares; P: 0; AE: GridExchange; XM:
          NoDXMults; QP: ARRLVHFJUNPointMethod; ADIFName: 'ARRL-VHF-JUN'; CABName:
          ''),
        ({Name: 'ARRL VHF SEP';                }Email: nil; DF: nil; WA7BNM:
          0000; {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: GridSquares; P: 0; AE:
          GridExchange; XM: NoDXMults; QP: ARRLVHFJUNPointMethod; ADIFName: '';
          CABName: ''),
        ({Name: 'BALTIC';                     }Email: nil; DF: nil; WA7BNM: 28;
          {SK3BG: nil;          }QRZRUID: 161; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: NoDXMults; QP: BalticQSOPointMethod; ADIFName:
          ''; CABName: ''),
        ({Name: 'BWQP';                       }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTNAMEANDPOSSIBLEFOCNUMBER; XM: NoDXMults; QP: BWQPQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'CIS';                        }Email: nil; DF: 'cis'; WA7BNM:
          0000; {SK3BG: nil;          }QRZRUID: 500; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTDomesticQTHOrQSONumberExchange; XM: ARRLDXCC; QP: CISQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'CQP';                        }Email: nil; DF: 'california_cty';
          WA7BNM: 0000; {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults;
          ZnM: NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 6; AE:
          QSONumberDomesticOrDXQTHExchange; XM: NoDXMults; QP: TwoPhoneThreeCW;
          ADIFName: ''; CABName: ''), //n4af
        ({Name: 'COUNTY HUNTER';              }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE: RSTQTHExchange;
          XM: NoDXMults; QP: OnePointPerQSO; ADIFName: ''; CABName: ''),
        ({Name: 'CQ-160-CW';                  }Email: '160CW@kkn.net'; DF:
          's48p14dc'; WA7BNM: 232; {SK3BG: 'cqww160';    }QRZRUID: 311; Pxm:
          NoPrefixMults; ZnM: NoZoneMults; AIE: ZoneInitialExchange; DM:
          DomesticFile; P: 0; AE: RSTZoneOrDomesticQTH; XM:
          CQDXCCWithNoUSAOrCanada; QP: CQ160QSOPointMethod; ADIFName: ''; CABName:
          ''), // 4.77.6
        ({Name: 'CQ-160-SSB';                 }Email: '160SSB@kkn.net'; DF:
          's48p14dc'; WA7BNM: 259; {SK3BG: 'cqww160';    }QRZRUID: 312; Pxm:
          NoPrefixMults; ZnM: NoZoneMults; AIE: ZoneInitialExchange; DM:
          DomesticFile; P: 0; AE: RSTZoneOrDomesticQTH; XM:
          CQDXCCWithNoUSAOrCanada; QP: CQ160QSOPointMethod; ADIFName: ''; CABName:
          ''), // 4.77.6
        ({Name: 'CQ-M';                       }Email: nil; DF: nil; WA7BNM: 14;
          {SK3BG: 'cqmidxc';    }QRZRUID: 126; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: CQDXCC {ARRLDXCC}; QP: CQMQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'CQ-VHF';                     }Email: nil; DF: nil; WA7BNM: 73;
          {SK3BG: 'cqwwvhf';    }QRZRUID: 363; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: GridSquares; P: 0; AE:
          RSTandorGridExchange; XM: NoDXMults; QP: CQVHFQSOPointMethod; ADIFName:
          ''; CABName: ''),
        ...
        ({Name: 'CQ-WPX-CW';                  }Email: 'cw@cqwpx.com'; DF: nil;
          WA7BNM: 29; {SK3BG: nil;          }QRZRUID: 18; Pxm: Prefix; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: NoDXMults; QP: CQWPXQSOPointMethod; ADIFName:
          ''; CABName: ''),
        ({Name: 'CQ-WPX-SSB';                 }Email: 'ssb@cqwpx.com'; DF: nil;
          WA7BNM: 291; {SK3BG: nil;          }QRZRUID: 6; Pxm: Prefix; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: NoDXMults; QP: CQWPXQSOPointMethod; ADIFName:
          ''; CABName: ''),
        ({Name: 'CQ-WPX-RTTY';                }Email: 'rtty@cqwpx.com'; DF: nil;
          WA7BNM: 245; {SK3BG: nil;          }QRZRUID: 6; Pxm: Prefix; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: NoDXMults; QP: CQWPXRTTYQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'CQ-WW-CW';                   }Email: 'cw@cqww.com'; DF: nil;
          WA7BNM: 192; {SK3BG: 'cqwwdxc';    }QRZRUID: 5; Pxm: NoPrefixMults; ZnM:
          CQZones; AIE: CustomInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTZoneExchange; XM: CQDXCC; QP: CQWWQSOPointMethod; ADIFName: '';
          CABName: ''), // 4.105.17
        ({Name: 'CQ-WW-SSB';                  }Email: 'ssb@cqww.com'; DF: nil;
          WA7BNM: 172; {SK3BG: 'cqwwdxc';    }QRZRUID: 4; Pxm: NoPrefixMults; ZnM:
          CQZones; AIE: ZoneInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTZoneExchange; XM: CQDXCC; QP: CQWWQSOPointMethod; ADIFName: '';
          CABName: ''),
        ({Name: 'CROATIAN';                   }Email: nil; DF: nil; WA7BNM: 206;
          {SK3BG: '9acwc';      }QRZRUID: 91; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: CQDXCC; QP: CroatianQSOPointMethod; ADIFName:
          ''; CABName: ''),
        ({Name: 'RF-CUP-CW';                  }Email: nil; {DF: 'grids';}WA7BNM:
          0000; {SK3BG: nil;          }QRZRUID: 27; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: GridFields; P: 0; AE:
          QSONumberAndGridSquare; XM: NoDXMults; QP: CupRFMethod; ADIFName: '';
          CABName: ''),
        ({Name: 'RF-CUP-SSB';                 }Email: nil; {DF: 'grids';}WA7BNM:
          0000; {SK3BG: nil;          }QRZRUID: 24; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: GridFields; P: 0; AE:
          QSONumberAndGridSquare; XM: NoDXMults; QP: CupRFMethod; ADIFName: '';
          CABName: ''),
        ({Name: 'RF-CUP-DIG';                 }Email: nil; DF: 'grids'; WA7BNM:
          0000; {SK3BG: nil;          }QRZRUID: 86; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          QSONumberAndGridSquare; XM: NoDXMults; QP: CupRFMethod; ADIFName: '';
          CABName: ''),
        ({Name: 'URAL-CUP';                   }Email: nil; DF: 'grids'; WA7BNM:
          0000; {SK3BG: nil;          }QRZRUID: 112; Pxm: CallSignPrefix; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          QSONumberDomesticQTHExchange; XM: NoDXMults; QP: OnePointPerQSO;
          ADIFName: ''; CABName: ''),
        ({Name: 'EU-SPRINT-SPRING-CW';        }Email: nil; DF: nil; WA7BNM: 317;
          QRZRUID: 216; Pxm: NoPrefixMults; ZnM: NoZoneMults; AIE:
          NameInitialExchange; DM: NoDomesticMults; P: 0; AE:
          QSONumberAndNameExchange; XM: NoDXMults; QP:
          EuropeanSprintQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'EUROPEAN HFC';               }Email: nil; DF: nil; WA7BNM: 82;
          {SK3BG: 'euhfcs';     }QRZRUID: 31; Pxm: NoPrefixMults; ZnM: EUHFCYear;
          AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE: RSTZoneExchange;
          XM: NoDXMults; QP: OnePointPerQSO; ADIFName: ''; CABName: ''),
        ({Name: 'EUROPEAN VHF';               }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberAndGridSquareExchange; XM: NoDXMults; QP:
          EuropeanVHFQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'TESLA      ';                }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberAndGridSquareExchange; XM: NoDXMults; QP:
          TeslaQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'ARRL FIELD DAY';             }Email: 'fieldday@arrl.org'; DF:
          'arrlsect'; WA7BNM: 57; {SK3BG: nil;          }QRZRUID: 0; Pxm:
          NoPrefixMults; ZnM: NoZoneMults; AIE: NoInitialExchange;
          {DM: NoDomesticMults;}P: 0; AE: ClassDomesticOrDXQTHExchange; XM:
          ARRLDXCC; QP: ARRLFieldDayQSOPointMethod; ADIFName: 'ARRL-FIELD-DAY';
          CABName: 'ARRL-FD'), //ny4i Issue 222
        ({Name: 'FISTS';                      }Email: nil; DF: 's49p8'; WA7BNM:
          251; {SK3BG: 'fistsspr';   }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTQTHNameAndFistsNumberOrPowerExchange; XM: NoDXMults; QP:
          FistsQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'FOC MARATHON';               }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;         }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE: RSTPowerExchange;
          XM: CQDXCC; QP: FOCMarathonQSOPointMethod; ADIFName: ''; CABName: ''),
          //n4af
        ({Name: 'FLORIDA QSO PARTY';          }Email: nil; DF: 'florida_cty';
          WA7BNM: 325; {SK3BG: 'flqp';       }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 11; AE:
          RSTDomesticQTHExchange; XM: NoDXMults; QP: OnePhoneTwoCW; ADIFName:
          'FL-QSO-PARTY'; CABName: ''), //N4AF 4.38.10
        ({Name: 'GACW-WWSA-CW';               }Email: nil; DF: nil; WA7BNM: 45;
          {SK3BG: 'gacwdxc';    }QRZRUID: 321; Pxm: NoPrefixMults; ZnM: CQZones;
          AIE: ZoneInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTZoneExchange; XM: CQDXCC; QP: GACWWWSACWQSOPointMethod; ADIFName: '';
          CABName: ''),
        ({Name: 'GAGARIN-CUP';                }Email: nil; DF: nil; WA7BNM: 367;
          {SK3BG: 'ygintc';     }QRZRUID: 82; Pxm: GCStation; ZnM: ITUZones; AIE:
          ZoneInitialExchange; DM: NoDomesticMults; P: 0; AE: RSTZoneExchange; XM:
          NoDXMults; QP: GagarinCupQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'GENERAL QSO';                }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NameQTHInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTNameAndQTHExchange; XM: NoDXMults; QP: OnePointPerQSO; ADIFName: '';
          CABName: ''),
        ({Name: 'GRID LOC';                   }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NameInitialExchange; DM: WYSIWYGDomestic; P: 0; AE:
          NameAndPossibleGridSquareExchange; XM: NoDXMults; QP: OnePointPerQSO;
          ADIFName: ''; CABName: ''),
        ({Name: 'HA DX';                      }Email: nil; DF: 'hungary';
          WA7BNM: 228; {SK3BG: '`c';      }QRZRUID: 116; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTDomesticQTHOrQSONumberExchange; XM: CQDXCC; QP: HADXQSOPointMethod;
          ADIFName: 'HA-DX'; CABName: ''), // 4.84.2
        ({Name: 'YUDX ';                      }Email: nil; DF: 'yu'; WA7BNM: 000;
          {SK3BG:  nil;         }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTDomesticQTHOrQSONumberExchange; XM: CQDXCC; QP: YUDXQSOPointMethod;
          ADIFName: ''; CABName: ''), // 4.57.5
        ({Name: 'UKEI ';                      }Email: nil; DF: 'uk-ei'; WA7BNM:
          000; {SK3BG:  nil;         }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTQSONUMBERAndPossibleDomesticQTHExchange; XM: CQDXCC; QP:
          UKEIQSOPointMethod; ADIFName: ''; CABName: ''), // 4.58.2
        ({Name: 'HELVETIA';                   }Email: nil; DF: 'swiss'; WA7BNM:
          326; {SK3BG: 'helvc';      }QRZRUID: 157; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTDomesticQTHOrQSONumberExchange; XM: NoDXMults; QP:
          HelvetiaQSOPointMethod; ADIFName: ''; CABName: ''), // 4.54.6 issue 214
        ({Name: 'IARU-HF';                    }Email: 'iaruhf@iaru.org'; DF:
          'iaruhq'; WA7BNM: 67; {SK3BG: 'iaruhfc';    }QRZRUID: 33; Pxm:
          NoPrefixMults; ZnM: ITUZones; AIE: ZoneInitialExchange; DM:
          WYSIWYGDomestic; P: 0; AE: RSTZoneOrSocietyExchange; XM: NoDXMults; QP:
          IARUQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'INTERNET SPRINT';            }Email: nil; DF: 's49p13'; WA7BNM:
          0000; {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          QSONumberNameDomesticOrDXQTHExchange; XM:
          NorthAmericanARRLDXCCWithNoUSACanadaOrkL7; QP: AlwaysOnePointPerQSO;
          ADIFName: ''; CABName: ''),
        ({Name: 'RSGB-IOTA';                  }Email: nil; DF: nil; WA7BNM: 75;
          {SK3BG: nil;          }QRZRUID: 29; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: IOTADomestic; P: 0; AE:
          RSTQSONumberAndPossibleDomesticQTHExchange; XM: NoDXMults; QP:
          IOTAQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'JIDX-CW';                    }Email: nil; DF: 'jidx'; WA7BNM:
          314; {SK3BG: 'jaintdx';    }QRZRUID: 57; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTZoneExchange; XM: NoDXMults; QP: JapanInternationalDXQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'JIDX-SSB';                   }Email: nil; DF: 'jidx'; WA7BNM:
          184; {SK3BG: 'jaintdx';    }QRZRUID: 57; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTZoneExchange; XM: NoDXMults; QP: JapanInternationalDXQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'JA LONG PREFECT';            }Email: nil; DF: 'jacg3'; WA7BNM:
          0000; {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTLongJAPrefectureExchange; XM: NoDXMults; QP: OnePointPerQSO; ADIFName:
          ''; CABName: ''),
        ({Name: 'JT DX';                      }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: MongolianCallSignPrefix; ZnM:
          NoZoneMults; AIE: ZoneInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTZoneExchange; XM: ARRLDXCCWithNoJT; QP: JTDXQSOPointMethod; ADIFName:
          ''; CABName: ''),
        ({Name: 'KCJ';                        }Email: nil; DF: 'japref'; WA7BNM:
          89; {SK3BG: 'kcjc';       }QRZRUID: 169; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTZONEORDOMESTICQTH; XM: NoDXMults; QP: KCJQSOPointMethod; ADIFName:
          ''; CABName: ''),
        ({Name: 'KIDS DAY';                   }Email: nil; DF: nil; WA7BNM: 224;
          {SK3BG: 'kidsday';    }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE: KidsDayExchange;
          XM: NoDXMults; QP: OnePointPerQSO; ADIFName: ''; CABName: ''),
        ({Name: 'KVP';                        }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: EUHFCYear;
          AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE: RSTZoneExchange;
          XM: NoDXMults; QP: OnePhoneTwoCW; ADIFName: ''; CABName: ''),
        ({Name: 'LZ DX';                      }Email: nil; DF: 'lz'; WA7BNM: 187;
          {SK3BG: 'lzdxc';      }QRZRUID: 53; Pxm: NoPrefixMults; ZnM: ITUZones;
          AIE: ZoneInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTZoneOrDomesticQTH; XM: NoDXMults; QP: LZDXQSOPointMethod; ADIFName:
          ''; CABName: ''),
        ({Name: 'Marconi Memorial';     }Email: 'contest.marconi@arifano.it';
          DF: nil; WA7BNM: 56; {SK3BG: nil;        }QRZRUID: 0; Pxm:
          NoPrefixMults; ZnM: NoZoneMults; AIE: NoInitialExchange; DM:
          NoDomesticMults; P: 0; AE: RSTQSONumberExchange; XM: CQDXCC; QP:
          OnePointPerQSO; ADIFName: ''; CABName: ''), //n4af    4.68.1
        ({Name: 'MINITEST';                   }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: CallSignPrefix; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: NoDXMults; QP: OnePointPerQSO; ADIFName: '';
          CABName: ''),
        ({Name: 'MINI80';                   }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: CallSignPrefix; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: NoDXMults; QP: OnePointPerQSO; ADIFName: '';
          CABName: ''),
        ({Name: 'MINI40';                   }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: CallSignPrefix; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: NoDXMults; QP: OnePointPerQSO; ADIFName: '';
          CABName: ''),
        ({Name: 'MICH QSO PARTY';             }Email: nil; DF: 'michigan_cty';
          WA7BNM: 323; {SK3BG: 'miqp';       }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 2; AE:
          RSTDomesticQTHExchange; XM: NoDXMults; QP: OnePhoneTwoCW; ADIFName: '';
          CABName: ''),
        ({Name: 'MINN QSO PARTY';             }Email: nil; DF: 'minnesota_cty';
          WA7BNM: 238; {SK3BG: 'mnqp';       }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 1; AE:
          NameAndDomesticOrDXQTHExchange; XM: NoDXMults; QP: TwoPointsPerQSO
          {MQPQSOPointMethod}; ADIFName: ''; CABName: ''),
        ({Name: 'NAQP-CW';                    }Email: 'cwnaqpmgr@ncjweb.com'; DF:
          'naqp'; WA7BNM: 218; {SK3BG: 'naqp';       }QRZRUID: 0; Pxm:
          NoPrefixMults; ZnM: NoZoneMults; AIE: NameInitialExchange; DM:
          DomesticFile; P: 0; AE: NameAndDomesticOrDXQTHExchange; XM:
          NorthAmericanARRLDXCCWithNoUSACanadaOrkL7; QP: OnePointPerQSO; ADIFName:
          ''; CABName: ''),
        ({Name: 'NAQP-SSB';                   }Email: 'ssbnaqpmgr@ncjweb.com';
          DF: 'naqp'; WA7BNM: 229; {SK3BG: 'naqp';       }QRZRUID: 0; Pxm:
          NoPrefixMults; ZnM: NoZoneMults; AIE: NameInitialExchange; DM:
          DomesticFile; P: 0; AE: NameAndDomesticOrDXQTHExchange; XM:
          NorthAmericanARRLDXCCWithNoUSACanadaOrkL7; QP: OnePointPerQSO; ADIFName:
          ''; CABName: ''),
        ({Name: 'NAQP-RTTY';                  }Email: 'rttynaqpmgr@ncjweb.com';
          DF: 'naqp'; WA7BNM: 263; {SK3BG: 'naqp';       }QRZRUID: 0; Pxm:
          NoPrefixMults; ZnM: NoZoneMults; AIE: NameInitialExchange; DM:
          DomesticFile; P: 0; AE: NameAndDomesticOrDXQTHExchange; XM:
          NorthAmericanARRLDXCCWithNoUSACanadaOrkL7; QP: OnePointPerQSO; ADIFName:
          ''; CABName: ''),
        ({Name: 'NA-SPRINT-CW';               }Email: 'cwsprint@ncjweb.com'; DF:
          'naqp'; WA7BNM: 253; {SK3BG: 'nasprint';   }QRZRUID: 0; Pxm:
          NoPrefixMults; ZnM: NoZoneMults; AIE: NameQTHInitialExchange; DM:
          DomesticFile; P: 0; AE: QSONumberNameDomesticOrDXQTHExchange; XM:
          NorthAmericanARRLDXCCWithNoUSACanadaOrkL7; QP: OnePointPerQSO; ADIFName:
          ''; CABName: ''),
        ({Name: 'SSB-SPRINT';                 }Email: ''; DF: 'naqp'; WA7BNM:
          5795; {SK3BG: '';   }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NameQTHInitialExchange; DM: DomesticFile; P: 0; AE:
          QSONumberNameDomesticOrDXQTHExchange; XM: NoDXMults; QP: OnePointPerQSO;
          ADIFName: ''; CABName: ''), // 4.55.10
        ({Name: 'NA-SPRINT-RTTY';             }Email: 'rttysprint@ncjweb.com';
          DF: 's49p8'; WA7BNM: 155; {SK3BG: 'nasprint';   }QRZRUID: 0; Pxm:
          NoPrefixMults; ZnM: NoZoneMults; AIE: NameQTHInitialExchange; DM:
          DomesticFile; P: 0; AE: QSONumberNameDomesticOrDXQTHExchange; XM:
          NorthAmericanARRLDXCCWithNoUSACanadaOrkL7; QP: OnePointPerQSO; ADIFName:
          ''; CABName: ''),
        ({Name: 'NCCC-SPRINT';                }Email: nil; DF: 'naqp'; WA7BNM:
          44; {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NameQTHInitialExchange; DM: DomesticFile; P: 0; AE:
          QSONumberNameDomesticOrDXQTHExchange; XM: NoDXMults; QP: OnePointPerQSO;
          ADIFName: ''; CABName: ''),
        ({Name: 'NEQP';                       }Email: nil; DF: nil; WA7BNM: 10;
          {SK3BG: 'newengqp';   }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTDomesticOrDXQTHExchange; XM: NoDXMults; QP: OnePhoneTwoCW; ADIFName:
          ''; CABName: ''),
        ({Name: 'NC QSO PARTY';               }Email: nil; DF: 'nc_cty'; WA7BNM:
          0000; {SK3BG:  nil ;       }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 13; AE:
          RSTDomesticQTHExchange; XM: NoDXMults; QP: NCQSOPointMethod; ADIFName:
          ''; CABName: ''),
        ({Name: 'NRAU-BALTIC-CW';             }Email: nil; DF: 'nrau'; WA7BNM:
          0000; {SK3BG: 'nrau';       }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTQSONumberAndDomesticQTHExchange; XM: NoDXMults; QP: TwoPointsPerQSO;
          ADIFName: ''; CABName: ''),
        ({Name: 'NRAU-BALTIC-SSB';            }Email: nil; DF: 'nrau'; WA7BNM:
          220; {SK3BG: 'nrau';       }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTQSONumberAndDomesticQTHExchange; XM: NoDXMults; QP: TwoPointsPerQSO;
          ADIFName: ''; CABName: ''),
        ({Name: 'NZ FIELD DAY';               }Email: nil; DF: nil; WA7BNM: 222;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: BranchZones;
          AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          NZFieldDayExchange; XM: NoDXMults; QP: NZFieldDayQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'OCEANIA-DX-CW';              }Email: nil; DF: nil; WA7BNM: 151;
          {SK3BG: 'ocdxc';      }QRZRUID: 72; Pxm: Prefix; ZnM: NoZoneMults; AIE:
          NoInitialExchange; DM: NoDomesticMults; P: 0; AE: RSTQSONumberExchange;
          XM: NoDXMults; QP: VKZLQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'OCEANIA-DX-SSB';             }Email: nil; DF: nil; WA7BNM: 142;
          {SK3BG: 'ocdxc';      }QRZRUID: 73; Pxm: Prefix; ZnM: NoZoneMults; AIE:
          NoInitialExchange; DM: NoDomesticMults; P: 0; AE: RSTQSONumberExchange;
          XM: NoDXMults; QP: VKZLQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'OHIO QSO PARTY';             }Email: nil; DF: 'ohio_cty';
          WA7BNM: 100; {SK3BG: 'ohqp';       }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 5; AE:
          RSTQSONumberAndDomesticQTHExchange; XM: NoDXMults; QP: OnePhoneTwoCW;
          ADIFName: ''; CABName: ''),
        ({Name: 'OK-OM DX';                   }Email: nil; DF: 'okom'; WA7BNM:
          185; {SK3BG: 'okomdxc';    }QRZRUID: 12; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTAndQSONumberOrDomesticQTHExchange; XM: NoDXMults; QP:
          OKDXQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'RADIO-ONY';                  }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 12; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTPowerExchange; XM: NoDXMults; QP: OldNewYearQSOPointMethod; ADIFName:
          ''; CABName: ''),
        ({Name: 'OZCR-O';                     }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 182; Pxm: NoPrefixMults; ZnM: ITUZones;
          AIE: ZoneInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTZoneOrSocietyExchange; XM: CQDXCC {ARRLDXCC}; QP: OnePointPerQSO;
          ADIFName: ''; CABName: ''),
        ({Name: 'OZCR-Z';                     }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 182; Pxm: NoPrefixMults; ZnM: ITUZones;
          AIE: ZoneInitialExchange; DM: WYSIWYGDomestic; P: 0; AE:
          RSTZoneOrSocietyExchange; XM: NoDXMults; QP: IARUQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'PACC';                       }Email: nil; DF: 'pacc'; WA7BNM:
          249; {SK3BG: 'pacc';       }QRZRUID: 66; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTAndQSONumberOrDomesticQTHExchange; XM: NoDXMults; QP: OnePointPerQSO;
          ADIFName: ''; CABName: ''),
        ({Name: 'QCWA';                       }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          QSONumberNameChapterAndQTHExchange; XM: NoDXMults; QP: OnePhoneTwoCW;
          ADIFName: ''; CABName: ''),
        ({Name: 'QCWA GOLDEN';                }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          QSONumberNameChapterAndQTHExchange; XM: NoDXMults; QP: OnePhoneTwoCW;
          ADIFName: ''; CABName: ''),
        ({Name: 'RAC_CANADA_WINTER';          }Email: nil; DF: 'p13'; WA7BNM:
          205; {SK3BG: 'canday';     }QRZRUID: 101; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTAndQSONumberOrDomesticQTHExchange; XM: NoDXMults; QP:
          RACQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'RF-VHF-FD   ';               }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 180; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberAndGridSquareExchange; XM: NoDXMults; QP:
          RadioVHFFDQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'RAEM';                       }Email: nil; DF: nil; WA7BNM: 209;
          {SK3BG: 'raem';       }QRZRUID: 88; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          QSONumberAndGeoCoordinates; XM: NoDXMults; QP: RAEMQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'RDAC';                       }Email: nil; DF: nil; WA7BNM: 94;
          {SK3BG: 'rdac';       }QRZRUID: 386; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: RDADistrict; P: 0; AE:
          RSTDomesticQTHOrQSONumberExchange; XM: ARRLDXCC; QP: RDAQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'REGION 1 FIELD DAY';         }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: CQDXCC; QP: EuropeanFieldDayQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'REGION 1 FIELD DAY-RCC-CW';  }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 358; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: CQDXCC; QP:
          RegionOneFieldDayRCCQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'REGION 1 FIELD DAY-RCC-SSB'; }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 358; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: CQDXCC; QP:
          RegionOneFieldDayRCCQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'RF-CHAMP-CW';                }Email: 'champ@srr.ru'; DF:
          'russian'; WA7BNM: 0000; {SK3BG: nil;          }QRZRUID: 30; Pxm:
          NoPrefixMults; ZnM: RFChampionchipZones; AIE: ZoneInitialExchange; DM:
          DomesticFile; P: 0; AE: QSONumberAndZone; XM: NoDXMults; QP:
          ChampionshipRFMethod; ADIFName: ''; CABName: ''),
        ({Name: 'RF-CHAMP-SSB';               }Email: 'champ@srr.ru'; DF:
          'russian'; WA7BNM: 0000; {SK3BG: nil;          }QRZRUID: 28; Pxm:
          NoPrefixMults; ZnM: RFChampionchipZones; AIE: ZoneInitialExchange; DM:
          DomesticFile; P: 0; AE: QSONumberAndZone; XM: NoDXMults; QP:
          ChampionshipRFMethod; ADIFName: ''; CABName: ''),
        ({Name: 'AS-CHAMP-CW';                }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 64; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          QSONumberAndCoordinatesSum; XM: NoDXMults; QP: ChampionshipRFASMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'RSGB-ROPOCO-CW';             }Email: nil; DF: nil; WA7BNM: 77;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTAndPostalCodeExchange; XM: NoDXMults; QP: TenPointsPerQSO; ADIFName:
          ''; CABName: ''),
        ({Name: 'RSGB-ROPOCO-SSB';            }Email: nil; DF: nil; WA7BNM: 361;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTAndPostalCodeExchange; XM: NoDXMults; QP: TenPointsPerQSO; ADIFName:
          ''; CABName: ''),
        ({Name: 'RSGB 1.8';                   }Email: nil; DF: 'rsgb'; WA7BNM:
          0000; {SK3BG: 'rsgb1-8';    }QRZRUID: 44; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTQSONumberAndPossibleDomesticQTHExchange; XM: CQDXCC; QP:
          RSGB160Method; ADIFName: ''; CABName: ''), // issue 296
        ({Name: 'RDXC';                       }Email: 'logs@rdxc.org'; DF:
          'russian'; WA7BNM: 310; {SK3BG: 'russdxc';    }QRZRUID: 7; Pxm:
          NoPrefixMults; ZnM: NoZoneMults; AIE: NoInitialExchange; DM:
          DomesticFile; P: 0; AE: RSTDomesticQTHOrQSONumberExchange; XM: ARRLDXCC;
          QP: RussianDXQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'SAC-CW';                     }Email: nil; DF: nil; WA7BNM: 121;
          {SK3BG: 'sacnsc';     }QRZRUID: 175; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: NoDXMults; QP: ScandinavianQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'YBDX';                       }Email: nil; DF: nil; WA7BNM: 121;
          {SK3BG: 'sacnsc';     }QRZRUID: 175; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: NoDXMults; QP: IndonesianQSOPointMethod;
          ADIFName: ''; CABName: ''), // 4.64.1
        ({Name: 'SAC-SSB';                    }Email: nil; DF: nil; WA7BNM: 131;
          {SK3BG: 'sacnsc';     }QRZRUID: 175; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: NoDXMults; QP: ScandinavianQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'SALMON RUN';                 }Email: nil; DF: 'washington_cty';
          WA7BNM: 126; {SK3BG: 'washsr';     }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 9; AE:
          RSTDomesticOrDXQTHExchange; XM: NoDXMults; QP: SalmonRunQSOPointMethod;
          ADIFName: ''; CABName: ''), // n4af 4.41.1
        ({Name: 'SOUTH AMERICAN WW';          }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTAndContinentExchange; XM: NoDXMults; QP:
          SouthAmericanWWQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'SP DX';                      }Email: nil; DF: 'spdx'; WA7BNM:
          312; {SK3BG: nil;          }QRZRUID: 127; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTDomesticQTHOrQSONumberExchange; XM: NoDXMults; QP: ThreePointsPerQSO;
          ADIFName: ''; CABName: ''),
        ({Name: 'STEW-PERRY';                 }Email: nil; DF: nil; WA7BNM: 207;
          {SK3BG: 'stpetbdc';   }QRZRUID: 46; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTAndOrGridExchange; XM: NoDXMults; QP: StewPerryQSOPointMethod;
          ADIFName: 'STEW-PERRY'; CABName: ''),
        ({Name: 'TEN TEN';                    }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          NameQTHAndPossibleTenTenNumber; XM: NoDXMults; QP: TenTenQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'TEXAS QSO PARTY';            }Email: nil; DF: 'texas_cty';
          WA7BNM: 133; {SK3BG: 'txqp';       }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 4; AE:
          RSTDomesticOrDXQTHExchange; XM: NoDXMults; QP: TwoPhoneThreeCW; ADIFName:
          'TX-QSO-PARTY'; CABName: ''), //n4af 4.33.6
        ({Name: 'TOEC';                       }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: 'toecwwgc';   }QRZRUID: 163; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: GridFields; P: 0; AE:
          RSTDomesticQTHExchange; XM: NoDXMults; QP: TOECQSOPointMethod; ADIFName:
          ''; CABName: ''),
        ({Name: 'UA4WCHAMPIONSHIP';           }Email: 'r4w@narod.ru'; DF: nil;
          WA7BNM: 0000; {SK3BG: nil;          }QRZRUID: 132; Pxm: NoPrefixMults;
          ZnM: NoZoneMults; AIE: NoInitialExchange; DM: WYSIWYGDomestic; P: 0; AE:
          RSTAndGridSquareOrRDAExchange; XM: NoDXMults; QP: UA4WMethod; ADIFName:
          ''; CABName: ''),
        ({Name: 'UBA-CW';                     }Email: nil; DF: 'uba'; WA7BNM:
          261; {SK3BG: 'ubac';       }QRZRUID: 59; Pxm: BelgiumPrefixes; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTQSONumberAndPossibleDomesticQTHExchange; XM: CQUBAEuropeanCountries;
          QP: UBAQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'UBA-SSB';                    }Email: nil; DF: 'uba'; WA7BNM:
          235; {SK3BG: 'ubac';       }QRZRUID: 58; Pxm: BelgiumPrefixes; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTQSONumberAndPossibleDomesticQTHExchange; XM: CQUBAEuropeanCountries;
          QP: UBAQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'UCG';                        }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 160; Pxm: Prefix; ZnM: NoZoneMults; AIE:
          NoInitialExchange; DM: NoDomesticMults; P: 0; AE: RSTQSONumberExchange;
          XM: NoDXMults; QP: CQWPXQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'UKRAINE CHAMPIONSHIP';       }Email: nil; DF: 'ukraine';
          WA7BNM: 0000; {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults;
          ZnM: NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          QSONumberDomesticQTHExchange; XM: NoDXMults; QP: ChampionshipUkrMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'UKRAINIAN';                  }Email: nil; DF: 'ukraine';
          WA7BNM: 176; {SK3BG: 'ukrdxc';     }QRZRUID: 8; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTAndQSONumberOrDomesticQTHExchange; XM: CQDXCC; QP:
          UkrainianQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'DARC-WAEDC-CW';              }Email: nil; DF: nil; WA7BNM: 85;
          {SK3BG: 'waedxc';     }QRZRUID: 15; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: NoDXMults; QP: WAEQSOPointMethod; ADIFName:
          ''; CABName: ''),
        ({Name: 'DARC-WAEDC-SSB';             }Email: nil; DF: nil; WA7BNM: 111;
          {SK3BG: 'waedxc';     }QRZRUID: 16; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: NoDXMults; QP: WAEQSOPointMethod; ADIFName:
          ''; CABName: ''),
        ({Name: 'DARC-XMAS';                  }Email: nil; DF: nil; WA7BNM: 210;
          {SK3BG: 'darcxmas';   }QRZRUID: 100; Pxm: Prefix; ZnM: NoZoneMults; AIE:
          NoInitialExchange; DM: WYSIWYGDomestic; P: 0; AE:
          RSTAndQSONumberOrDomesticQTHExchange; XM: NoDXMults; QP: OnePointPerQSO;
          ADIFName: ''; CABName: ''),
        ({Name: 'WAG';                        }Email: nil; DF: nil; WA7BNM: 163;
          {SK3BG: 'wadlc';      }QRZRUID: 74; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTAndQSONumberOrDomesticQTHExchange; XM: NoDXMults; QP:
          WAGQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'WISCONSIN QSO PARTY';        }Email: nil; DF: 'wisconsin_cty';
          WA7BNM: 330; {SK3BG: 'wisqp';      }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 7; AE:
          RSTDomesticQTHExchange; XM: NoDXMults; QP: OnePhoneTwoCW; ADIFName:
          'WI-QSO-PARTY'; CABName: ''),
        ({Name: 'WWL';                        }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: NoInitialExchange; DM: GridFields; P: 0; AE: RSTAndGridExchange;
          XM: NoDXMults; QP: WWLQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'WW PMC';                     }Email: nil; DF: 'pmc'; WA7BNM:
          471; {SK3BG: nil;          }QRZRUID: 229; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTZoneOrSocietyExchange; XM: NoDXMults; QP: WWPMCQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'XMAS';                       }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 100; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberAndRandomCharactersExchange; XM: NoDXMults; QP:
          TwoPointsPerQSO; ADIFName: ''; CABName: ''),
        ({Name: 'YO DX';                      }Email: nil; DF: 'romania';
          WA7BNM: 98; {SK3BG: 'yodxc';      }QRZRUID: 328; Pxm: NoPrefixMults;
          ZnM: NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTAndQSONumberOrDomesticQTHExchange; XM: ARRLDXCC; QP:
          YODXQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'SRR-JR';                     }Email: nil; DF: 'russian';
          WA7BNM: 0000; {SK3BG: nil;          }QRZRUID: 331; Pxm: NoPrefixMults;
          ZnM: NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          AgeAndQSONumberExchange; XM: ARRLDXCC; QP: AlwaysOnePointPerQSO;
          ADIFName: ''; CABName: ''), // n4af 04.42.6
        ({Name: 'RU3AX MEMORIAL';                  }Email: nil; DF: 'russian';
          WA7BNM: 202; {SK3BG: 'russ160';    }QRZRUID: 90; Pxm: NoPrefixMults;
          ZnM: NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTDomesticQTHOrQSONumberExchange; XM: ARRLDXCC; QP:
          RussianDXQSOPointMethod; ADIFName: ''; CABName: ''), // 4.65.4
        ({Name: 'LOCUST QP';                  }Email: nil; DF: 'naqp'; WA7BNM:
          446; {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NameQTHInitialExchange; DM: DomesticFile; P: 0; AE:
          NameAndDomesticOrDXQTHExchange {QSONumberNameDomesticOrDXQTHExchange};
          XM: NoDXMults; QP: LQPQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'ARKTIKA-SPRING';             }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 351; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: WYSIWYGDomestic; P: 0; AE:
          RSTAndQSONumberOrDomesticQTHExchange; XM: NoDXMults; QP:
          ArktikaSpringQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'UN DX';                      }Email: nil; DF: 'kda'; WA7BNM:
          0000; {SK3BG: nil;          }QRZRUID: 13; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTDomesticQTHOrQSONumberExchange; XM: ARRLDXCC; QP: UNDXQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'NYQP';                       }Email: nil; DF: 'newyork_cty';
          WA7BNM: 473; {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 3; AE:
          RSTDomesticQTHExchange; XM: NoDXMults; QP: OnePhoneTwoCW; ADIFName: '';
          CABName: ''),
        ({Name: 'KING-OF-SPAIN-CW';           }Email: nil; DF: 'ea'; WA7BNM: 23;
          {SK3BG: 'kingofsp';   }QRZRUID: 308; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTDomesticQTHOrQSONumberExchange; XM: CQDXCC; QP:
          KingOfSpainQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'KING-OF-SPAIN-SSB';          }Email: nil; DF: 'ea'; WA7BNM: 59;
          {SK3BG: 'kingofsp';   }QRZRUID: 308; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTDomesticQTHOrQSONumberExchange; XM: CQDXCC; QP:
          KingOfSpainQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'WRTC';                       }Email: nil; DF: 'iaruhq'; WA7BNM:
          0000; {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: ZoneInitialExchange; DM: WYSIWYGDomestic; P: 0; AE:
          RSTZoneOrSocietyExchange; XM: ARRLDXCC; QP: WRTCQSOPointMethod; ADIFName:
          ''; CABName: ''), //n4af 4.31.2
        ({Name: 'TENNESSEE QSO PARTY';        }Email: nil; DF: 'tennessee_cty';
          WA7BNM: 115; {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 8; AE:
          RSTDomesticQTHExchange; XM: NoDXMults; QP: TwoPhoneThreeCW; ADIFName:
          'TN-QSO-PARTY'; CABName: ''), // n4af 4.51.4
        ({Name: 'COLORADO QSO PARTY';         }Email: 'colorado_cty'; DF: nil;
          WA7BNM: 431; {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 12; AE:
          NameAndDomesticOrDXQTHExchange; XM: NoDXMults; QP: OnePhoneTwoCW;
          ADIFName: 'CO-QSO-PARTY'; CABName: ''),
        ({Name: 'R9W-UW9WK-MEMORIAL';         }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 41; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          QSONumberAndZone; XM: NoDXMults; QP: R9WUW9WKMemorialQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'PCC';                        }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 35; Pxm: Prefix; ZnM: NoZoneMults; AIE:
          NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTAndQSONumberOrDomesticQTHExchange; XM: NoDXMults; QP:
          PCCQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'RADIO-MEMORY';               }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 83; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTAgeAndPossibleSK; XM: NoDXMults; QP: RadioMemoryQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'DARC-10M';                   }Email: nil; DF: nil; WA7BNM: 223;
          {SK3BG: 'darcxmas';   }QRZRUID: 184; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: WYSIWYGDomestic; P: 0; AE:
          RSTQSONumberAndPossibleDomesticQTHExchange {RSTAndQSONumberOrDomesticQTHExchange};
          XM: CQDXCC; QP: OnePointPerQSO; ADIFName: ''; CABName: ''),
        ({Name: 'REF-CW';                     }Email: nil; DF: 'ref'; WA7BNM:
          233; {SK3BG: 'refc';       }QRZRUID: 67; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTAndQSONumberOrFrenchDepartmentExchange; XM: NoDXMults; QP:
          REFQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'REF-SSB';                    }Email: nil; DF: 'ref'; WA7BNM:
          260; {SK3BG: 'refc';       }QRZRUID: 67; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTAndQSONumberOrFrenchDepartmentExchange; XM: NoDXMults; QP:
          REFQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'BSCI';                       }Email: nil; DF: nil; WA7BNM: 470;
          {SK3BG: nil;          }QRZRUID: 232; Pxm: NoPrefixMults; ZnM: ITUZones;
          AIE: ZoneInitialExchange; DM: WYSIWYGDomestic; P: 0; AE:
          RSTZoneOrSocietyExchange; XM: BlackSeaCountries; QP: BSCIQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'CQMM';                       }Email: nil; DF: nil; WA7BNM: 21;
          {SK3BG: nil;          }QRZRUID: 34; Pxm: SouthAmericanPrefixes; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTAndContinentExchange; XM: CQDXCC; QP: CQMMQSOPointMethod; ADIFName:
          ''; CABName: ''),
        ({Name: 'SA-SPRINT';                  }Email:
          'sa-sprint.com/enviar-log/'; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: SouthAmericanPrefixes; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          RSTQSONumberExchange; XM: NoDXMults; QP: OnePointPerQSO; ADIFName: '';
          CABName: ''), // 4.50.5 issue 177
        ({Name: 'CWOPS';                      }Email: nil; DF: nil; WA7BNM: 498;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: CallSignPrefix; ZnM:
          NoZoneMults; AIE: NameQTHInitialExchange; DM: NoDomesticMults; P: 0; AE:
          NameAndDomesticOrDXQTHExchange; XM: NoDXMults; QP: OnePointPerQSO;
          ADIFName: 'CWOPS-CWT'; CABName: ''),
        ({Name: 'OZHCR-VHF';                  }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 203; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: GridSquares; P: 0; AE:
          RSTQSONumberAndGridSquareExchange; XM: NoDXMults; QP:
          OZHCRVHFQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'RAC_CANADA_DAY';             }Email: nil; DF: 'p13'; WA7BNM: 60;
          {SK3BG: 'canday';     }QRZRUID: 101; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTAndQSONumberOrDomesticQTHExchange; XM: NoDXMults; QP:
          RACQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'CQ-WW-RTTY';                 }Email: 'rtty@cqww.com'; DF:
          's48p14dc'; WA7BNM: 130; {SK3BG: nil;          }QRZRUID: 191; Pxm:
          NoPrefixMults; ZnM: CQZones; AIE: ZoneInitialExchange; DM: DomesticFile;
          P: 0; AE: RSTZoneAndPossibleDomesticQTHExchange; XM: CQDXCC; QP:
          CQWWRTTYQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'CW-OPEN';                     }Email: 'cwo@cwops.org'; DF: nil;
          WA7BNM: 532; {SK3BG: nil;          }QRZRUID: 0; Pxm: CallSignPrefix;
          ZnM: CQZones; AIE: NameInitialExchange; DM: NoDomesticMults; P: 0; AE:
          QSONumberAndNameExchange; XM: NoDXMults; QP: OnePointPerQSO; ADIFName:
          'CWOPS-CW-OPEN'; CABName: 'CW-OPEN'),
        ({Name: 'Makrothen';                  }Email: nil; DF: nil; WA7BNM: 159;
          {SK3BG: nil;          }QRZRUID: 515; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: GridInitialExchange; DM: NoDomesticMults; P: 0; AE:
          GridExchange; XM: NoDXMults; QP: MakrothenQSOPointMethod; ADIFName: '';
          CABName: ''),
        ({Name: 'EU-SPRINT-SPRING-SSB';       }Email: nil; DF: nil; WA7BNM: 316;
          QRZRUID: 216; Pxm: NoPrefixMults; ZnM: NoZoneMults; AIE:
          NameInitialExchange; DM: NoDomesticMults; P: 0; AE:
          QSONumberAndNameExchange; XM: NoDXMults; QP:
          EuropeanSprintQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'EU-SPRINT-AUTUMN-CW';        }Email: nil; DF: nil; WA7BNM: 152;
          QRZRUID: 216; Pxm: NoPrefixMults; ZnM: NoZoneMults; AIE:
          NameInitialExchange; DM: NoDomesticMults; P: 0; AE:
          QSONumberAndNameExchange; XM: NoDXMults; QP:
          EuropeanSprintQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'EU-SPRINT-AUTUMN-SSB';       }Email: nil; DF: nil; WA7BNM: 143;
          QRZRUID: 216; Pxm: NoPrefixMults; ZnM: NoZoneMults; AIE:
          NameInitialExchange; DM: NoDomesticMults; P: 0; AE:
          QSONumberAndNameExchange; XM: NoDXMults; QP:
          EuropeanSprintQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'ARRL RTTY ROUNDUP';       }Email: 'rttyru@arrl.org'; DF:
          's48p14dc'; WA7BNM: 217; QRZRUID: 56; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; DM: DomesticFile; P: 0; AE:
          RSTDomesticQTHOrQSONumberExchange; XM: ARRLDXCC; QP: OnePointPerQSO;
          ADIFName: 'ARRL-RTTY'; CABName: ''),
        ({Name: 'CQIR';                    }Email: nil; DF: 'ireland'; WA7BNM:
          434; QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults; DM: DomesticFile;
          P: 0; AE: QSONumberAndPossibleDomesticQTHExchange; XM: NoDXMults; QP:
          TwoPhoneThreeCW; ADIFName: ''; CABName: ''),
        ({Name: 'WWIH';                    }Email: nil; DF: nil; WA7BNM: 552;
          QRZRUID: 0; Pxm: NoPrefixMults; ZnM: CQZones; AIE: ZoneInitialExchange;
          DM: NoDomesticMults; P: 0; AE: RSTZoneExchange; XM: CQDXCC; QP:
          CQWWRTTYQSOPointMethod; ADIFName: ''; CABName: ''),
        ({Name: 'ALRS_UA1DZ_CUP';          }Email: nil; DF: 'russian'; WA7BNM:
          0000; QRZRUID: 543; Pxm: NoPrefixMults; ZnM: NoZoneMults; AIE:
          NoInitialExchange; DM: WYSIWYGDomestic; P: 0; AE:
          RSTDomesticQTHExchange; XM: CQDXCC; QP: ALRSUA1DZCupQSOPointMethod;
          ADIFName: ''; CABName: ''),
        ({Name: 'WINTER FIELD DAY';        }Email: 'wfda@winterfieldday.com'; DF:
          'arrlsect'; WA7BNM: 421; {SK3BG: nil;         }QRZRUID: 0; Pxm:
          NoPrefixMults; ZnM: NoZoneMults; AIE: NoInitialExchange; DM:
          DomesticFile; P: 0; AE: ClassDomesticOrDXQTHExchange; XM: ARRLDXCC; QP:
          ARRLFieldDayQSOPointMethod; ADIFName: 'WFD'; CABName: 'WFD'),
        ({Name: 'RADIO-YOC';                  }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: nil;          }QRZRUID: 119; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: NoDomesticMults; P: 0; AE:
          QSONumberAndPreviousQSONumber; XM: ARRLDXCC; QP: ThreePointsPerQSO;
          ADIFName: ''; CABName: ''),
        ({Name: 'PA QSO PARTY';               }Email: nil; DF: 'pa_cty'; WA7BNM:
          0000; {SK3BG:  nil ;        }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 14; AE:
          QSONumberDomesticQTHExchange; XM: NoDXMults; QP: PAQSOPointMethod;
          ADIFName: 'PA-QSO-PARTY'; CABName: ''), // 4.74.3
        ({Name: 'IN QSO PARTY';               }Email: nil; DF: 'in_cty'; WA7BNM:
          0000; {SK3BG:  nil ;        }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 15; AE:
          RSTDomesticQTHExchange; XM: NoDXMults; QP: PAQSOPointMethod; ADIFName:
          'IN-QSO-PARTY'; CABName: ''), // 4.88.2
        ({Name: 'OK-OM SSB';                  }Email: nil; DF: 'okom'; WA7BNM:
          185; {SK3BG: 'okomdxc';    }QRZRUID: 12; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTAndQSONumberOrDomesticQTHExchange; XM: CQDXCC; QP:
          OKOMSSBQSOPointMethod; ADIFName: ''; CABName: ''), // 4.80.1
        ({Name: 'BATAVIA-FT8';                }Email: nil; DF: nil; WA7BNM:
          10686; QRZRUID: 0; Pxm: Prefix; ZnM: NoZoneMults; AIE:
          GridInitialExchange; DM: NoDomesticMults; P: 0; AE: GridExchange; XM:
          CQDXCC; QP: YBFT8QP; ADIFName: ''; CABName: 'BATAVIA'), // 4.90.11
        ({Name: 'WWDIGI';                     }Email: 'director@ww-digi.com';
          DF: nil; WA7BNM: 650; QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults;
          AIE: GridInitialExchange; DM: GridFields; P: 0; AE: Grid2Exchange; XM:
          NoDXMults; QP: WWDIGIQP; ADIFName: ''; CABName: 'WW-DIGI'), // 4.90.11
        ({Name: 'MWC';                        }Email:
          'memorial-ok1wc.cz/index.php?page=logs'; DF: 'mwc'; WA7BNM: 0; QRZRUID:
          0; Pxm: NoPrefixMults; ZnM: NoZoneMults; AIE: NoInitialExchange; DM:
          DomesticFile; P: 0; AE: RSTQSONumberExchange; XM: NoDXMults; QP: MWCQP;
          ADIFName: ''; CABName: ''), // 4.98.1
        ({Name: 'IRTS ';                      }Email: 'IRTS.contests@gmail.com';
          DF: 'ireland'; WA7BNM: 000; {SK3BG:  nil;         }QRZRUID: 0; Pxm:
          NoPrefixMults; ZnM: NoZoneMults; AIE: ZoneInitialExchange; DM:
          DomesticFile; P: 0; AE: RSTZoneOrDomesticQTH; XM: NoDXMults; QP:
          EUDXQSOPointMethod; ADIFName: ''; CABName: ''), // 4.95.6
        ({Name: 'EUDX';                       }Email: 'log.eudxc@gmail.com'; DF:
          'EUDX'; WA7BNM: 000; {SK3BG: nil;    }QRZRUID: 0; Pxm: NoPrefixMults;
          ZnM: NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 0; AE:
          RSTZoneOrDomesticQTH; XM: CQDXCC; QP: EUDXQSOPointMethod; ADIFName: '';
          CABName: ''),
        ({Name: 'BCQP';                       }Email: nil; DF: 've7_cty';
          WA7BNM: 473; {SK3BG: nil;          }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 16; AE:
          RSTDomesticQTHExchange; XM: NoDXMults; QP: BCQPQSOPointMethod; ADIFName:
          ''; CABName: ''), // 4.97.6
        ({Name: 'VA QSO Party';               }Email: nil; DF: 'va_cty'; WA7BNM:
          0000; {SK3BG:  nil ;        }QRZRUID: 0; Pxm: NoPrefixMults; ZnM:
          NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 17; AE:
          QSONumberDomesticOrDXQTHExchange; XM: NoDXMults; QP: VAQSOPointMethod;
          ADIFName: 'VA-QSO-PARTY'; CABName: ''), // 4.88.2
        ({Name: 'YOTA';                       }Email: nil; DF: 'YOTA'; WA7BNM: 0;
          {SK3BG: nil;    }QRZRUID: 0; Pxm: NoPrefixMults; ZnM: NoZoneMults; AIE:
          NoInitialExchange; DM: DomesticFile; P: 0; AE: RSTAgeExchange; XM:
          NoDXMults; QP: YOTAQSOPointMethod; ADIFName: 'YOTA'; CABName: ''),
        ({Name: 'IN7QPNE';                    }Email: nil; DF: 'in7qpne_cty';
          WA7BNM: 0000; {SK3BG:  nil ;        }QRZRUID: 0; Pxm: NoPrefixMults;
          ZnM: NoZoneMults; AIE: NoInitialExchange; DM: DomesticFile; P: 18; AE:
          RSTDomesticQTHExchange; XM: NoDXMults; QP: PAQSOPointMethod; ADIFName:
          'IN7QPNE-QSO-PARTY'; CABName: ''), // 4.88.2
        ({Name: 'MST';                        }Email: nil; DF: nil; WA7BNM: 3146;
          {SK3BG: nil;          }QRZRUID: 0; Pxm: CallSignPrefix; ZnM:
          NoZoneMults; AIE: NameInitialExchange; DM: NoDomesticMults; P: 0; AE:
          QSONumberandNameExchange; XM: NoDXMults; QP: OnePointPerQSO; ADIFName:
          'MST'; CABName: ''), // 4.110.5
        ({Name: 'RSGB-DX';                   }Email: nil; DF: nil; WA7BNM: 0000;
          {SK3BG: 'rsgb1-8';    }QRZRUID: 44; Pxm: Prefix; ZnM: NoZoneMults; AIE:
          NoInitialExchange; DM: NoDomesticMults; P: 0; AE: RSTQSONumberExchange;
          XM: nodxmults; QP: RSGBDXMethod; ADIFName: ''; CABName: '') // 5.115.6
        );
    ...
    const
      ...
      ContestsBooleanArray: array[ContestType] of Byte =
        (
        ({Name: 'DUMMY CONTEST';              }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB0 + ciQM0 + ciMB0 + ciMM0),
        ({Name: '7QP';                        }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled1 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0),
        ({Name: 'ALL ASIAN CW';               }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'ALL ASIAN SSB';              }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'ALL JA';                     }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled1 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'AP-SPRINT';                  }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'ARCI';                       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'ARI-DX';                     }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'ARRL-10';                    }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB0 + ciQM1 + ciMB0 + ciMM1),
        ({Name: 'ARRL-DIGI';                  }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'ARRL-160';                   }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB0 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'ARRL-DX-CW';                 }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'ARRL-DX-SSB';                }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'ARRL-SS-CW';                 }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB0 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'ARRL-SS-SSB';                }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB0 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'ARRL VHF JAN';               }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled1 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'ARRL VHF JUN';               }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled1 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'ARRL VHF SEP';                }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled1 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'BALTIC';                     }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB0 + ciQM1 + ciMB0 + ciMM0),
        ({Name: 'BWQP';                       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0), //n4af
        ({Name: 'CIS';                        }ciCDC1 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'CQP';                        }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0), //n4af 4.34.5
        ({Name: 'COUNTY HUNTER';              }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB0 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'CQ-160-CW';                  }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB0 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'CQ-160-SSB';                 }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB0 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'CQ-M';                       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'CQ-VHF';                     }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled1 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'CQ-WPX-CW';                  }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'CQ-WPX-SSB';                 }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'CQ-WPX-RTTY';                }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'CQ-WW-CW';                   }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'CQ-WW-SSB';                  }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'CROATIAN';                   }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'RF-CUP-CW';                  }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'RF-CUP-SSB';                 }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'RF-CUP-DIG';                 }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'URAL-CUP';                   }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'EU-SPRINT-SPRING-CW';        }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'EUROPEAN HFC';               }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'EUROPEAN VHF';               }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled1 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'TESLA       ';               }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0), // 4.57.4
        ({Name: 'ARRL-FIELD DAY';             }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled1 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0),
        ({Name: 'FISTS';                      }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'FOC MARATHON';               }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0), //n4af
        ({Name: 'FLORIDA QSO PARTY';          }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM1),
        ({Name: 'GACW-WWSA-CW';               }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'GAGARIN-CUP';                }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'GENERAL QSO';                }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0),
        ({Name: 'GRID LOC';                   }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'HA DX';                      }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'YUDX';                       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
          // 4.57.5    // 4.58.1
        ({Name: 'UKEI';                       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0), // 4.58.2
        ({Name: 'HELVETIA';                   }ciCDC1 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
          // 4.54.6 issue 214    // 4.56.8
        ({Name: 'IARU-HF';                    }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'INTERNET SPRINT';            }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'RSGB-IOTA';                  }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM1),
        ({Name: 'JIDX-CW';                    }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'JIDX-SSB';                   }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'JA LONG PREFECT';            }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB0 + ciQM1 + ciMB0 + ciMM0),
        ({Name: 'JT DX';                      }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'KCJ';                        }ciCDC1 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'KIDS DAY';                   }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB0 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'KVP';                        }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB0 + ciQM1 + ciMB1 + ciMM1),
        ({Name: 'LZ DX';                      }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name; 'Marconi Memorial';           }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0), //n4af
        ({Name: 'MINITEST';                   }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'MINI80';                   }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'MINI40';                   }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'MICH QSO PARTY';             }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM1),
        ({Name: 'MINN QSO PARTY';             }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled1 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0),
        ({Name: 'NAQP-CW';                    }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'NAQP-SSB';                   }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'NAQP-RTTY';                  }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'NA-SPRINT-CW';               }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'SSB SPRINT';                 }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'NA-SPRINT-RTTY';             }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'NCCC-SPRINT';                }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'NEQP';                       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0),
        ({Name: 'NC QSO PARTY';               }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0),
          // issue 292 4.66.1
        ({Name: 'NRAU-BALTIC-CW';             }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'NRAU-BALTIC-SSB';            }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'NZ FIELD DAY';               }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM1),
        ({Name: 'OCEANIA-DX-CW';              }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM1),
        ({Name: 'OCEANIA-DX-SSB';             }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM1),
        ({Name: 'OHIO QSO PARTY';             }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM1),
        ({Name: 'OK-OM DX';                   }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'RADIO-ONY';                  }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM1 + ciMB0 + ciMM0),
        ({Name: 'OZCR-O';                     }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'OZCR-Z';                     }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'PACC';                       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'QCWA';                       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0),
        ({Name: 'QCWA GOLDEN';                }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB0 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'RAC CANADA WINTER';          }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled1 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM1),
        ({Name: 'RF-VHF-FD';                  }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled1 + ciErmak1 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'RAEM';                       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'RDAC';                       }ciCDC1 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'REGION 1 FIELD DAY';         }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'REGION 1 FIELD DAY-RCC-CW';  }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'REGION 1 FIELD DAY-RCC-SSB'; }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'RF-CHAMP-CW';                }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'RF-CHAMP-SSB';               }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'AS-CHAMP-CW';                }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM1 + ciMB0 + ciMM0),
        ({Name: 'RSGB-ROPOCO-CW';             }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'RSGB-ROPOCO-SSB';            }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'RSGB 1.8';                   }ciCDC1 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB0 + ciQM0 + ciMB0 + ciMM0), // issue 296
        ({Name: 'RDXC';                       }ciCDC1 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'SAC-CW';                     }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'YBDX';                       }ciCDC1 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
          // 4.64.1   // add issue 291
        ({Name: 'SAC-SSB';                    }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'SALMON RUN';                 }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0),
        ({Name: 'SOUTH AMERICAN WW';          }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'SP DX';                      }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'STEW-PERRY';                 }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB0 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'TEN TEN';                    }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0),
        ({Name: 'TEXAS QSO PARTY';            }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0),
        ({Name: 'TOEC';                       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'R4W-CHAMP';                  }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'UBA-CW';                     }ciCDC1 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'UBA-SSB';                    }ciCDC1 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'UCG';                        }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'UKRAINE CHAMPIONSHIP';       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'UKRAINIAN';                  }ciCDC1 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'DARC-WAEDC-CW';              }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'DARC-WAEDC-SSB';             }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'DARC-XMAS';                  }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'WAG';                        }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'WISCONSIN QSO PARTY';        }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0),
        ({Name: 'WWL';                        }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'WW PMC';                     }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM1),
        ({Name: 'XMAS';                       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'YO DX';                      }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'SRR-JR';                     }ciCDC1 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'RU3AX MEMORIAL';             }ciCDC1 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB0 + ciQM1 + ciMB0 + ciMM1),
          // 4.65.    // 4.76.5
        ({Name: 'LQP';                        }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'ARKTIKA-SPRING';             }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'UN DX';                      }ciCDC1 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'NYQP';                       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM1),
        ({Name: 'KING-OF-SPAIN-CW';           }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'KING-OF-SPAIN-SSB';          }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'WRTC';                       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'TENNESSEE QSO PARTY';        }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'COLORADO QSO PARTY';         }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM1),
        ({Name: 'R9W-UW9WK-MEMORIAL';         }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'PCC';                        }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'RADIO-MEMORY';               }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM1 + ciMB0 + ciMM0),
        ({Name: 'DARC-10M';                   }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB0 + ciQM1 + ciMB0 + ciMM0),
        ({Name: 'REF-CW';                     }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'REF-SSB';                    }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'BSCI';                       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'CQMM';                       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'SA-SPRINT';                  }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
          // 4.50.5 issue 177
        ({Name: 'CWOPS';                      }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'OZHCR-VHF';                  }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled1 + ciErmak1 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'RAC CANADA WINTER';          }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled1 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM1),
        ({Name: 'CQ-WW-CW';                   }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'CWOPEN';                     }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'MAKROTHEN';                  }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'EU-SPRINT-SPRING-SSB';       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'EU-SPRINT-AUTUMN-CW';        }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'EU-SPRINT-AUTUMN-SSB';       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'ARRL RTTY ROUNDUP';          }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'CQIR';                       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM1),
        ({Name: 'WWIH';                       }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM1),
        ({Name: 'ALRS-UA1DZ-CUP';             }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'WINTER-FIELD DAY';           }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled1 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0), // 4.56.9 ny4i
        ({Name: 'RADIO-YOC';                  }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak1 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'PA QSO PARTY';               }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0), // 4.74.3
        ({Name: 'IN QSO PARTY';               }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0), // 4.88.2
        ({Name: 'OK-OM SSB';                  }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'BATAVIA-FT8';                }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0),
        ({Name: 'WW-DIGI';                    }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB1 + ciMM0),
        ({Name: 'MWC';                        }ciCDC0 + ciCQZoneMode1 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0), // 4.92.8
        ({Name: 'IRTS';                       }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0), // 4.93.1
        ({Name: 'EUDX';                       }ciCDC1 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0), // 4.95.6
        ({Name: 'BC QSO Party';               }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM1), // 4.97.6
        ({Name: 'VA QSO PARTY';               }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0),
        ({Name: 'YOTA';                       }ciCDC1 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM0),
        ({Name: 'IN7QPNE     ';               }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0), // 4.99.7
        ({Name: 'MST';                        }ciCDC0 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM0 + ciMB0 + ciMM0), // 4.110.5
        ({Name: 'RSGB-DX';                    }ciCDC1 + ciCQZoneMode0 +
          ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB1 + ciMM1)
    ...
    const SizeOfLHValid = (SizeOfLHVersionString + SizeOfLHFileDesc +
      SizeOfLHWarningString);
    ...
    type TLogHeader = record
        ...
        lhDummy: array[0..(SizeOfContestExchange - SizeOfLHValid - 1)] of Char;
          // three fields above are 60 bytes
    ...
    const
      LogHeader: TLogHeader =
        (
        lhVersionString: (LOGVERSION1, LOGVERSION2, LOGVERSION3, LOGVERSION4,
          #0, #$20, #13, #10);
        lhFileDesc: ('T', 'R', '4', 'W', ' ', 'L', 'O', 'G', ' ', 'F', 'I', 'L',
          'E', ' ', #13, #10);
        lhWarningString: ('W', 'A', 'R', 'N', 'I', 'N', 'G', ':', ' ', 'D', 'O',
          ' ', 'N', 'O', 'T', ' ', 'E', 'D', 'I', 'T', ' ', 'T', 'H', 'I', 'S',
          ' ', 'F', 'I', 'L', 'E', '!', #13, #10, #13, #10, #0);
        );
