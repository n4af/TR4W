...
{$IFEND}
  ...
  TR4W_CURRENTVERSION_NUMBER            = '4.56.1';     // N4af     New Release


  TR4W_CURRENTVERSION                   = 'TR4W v.' + TR4W_CURRENTVERSION_NUMBER; //  {$IF MMTTYMODE} + '_mmtty'{$IFEND};//{$IF LANG <> 'ENG'} + ' [' + LANG + ']'{$IFEND}{$IF MMTTYMODE} + '_mmtty'{$IFEND};
  TR4W_CURRENTVERSIONDATE               = 'January 31, 2016' ;
 ...
 {*)}
  ...
  const
    ...
    ContestsBooleanArray                : array[ContestType] of Byte =
      ...
      ({Name: 'HELVETIA';                   }ciCDC0 + ciCQZoneMode0 + ciVHFEnabled0 + ciErmak0 + ciQB1 + ciQM1 + ciMB0 + ciMM0),    // 4.54.6 issue 214
