...
{$IFEND}
  ...
  TR4W_CURRENTVERSION_NUMBER            = '4.73.3' ;  // N4af     New Release


  TR4W_CURRENTVERSION                   = 'TR4W v.' + TR4W_CURRENTVERSION_NUMBER; //  {$IF MMTTYMODE} + '_mmtty'{$IFEND};//{$IF LANG <> 'ENG'} + ' [' + LANG + ']'{$IFEND}{$IF MMTTYMODE} + '_mmtty'{$IFEND};
  TR4W_CURRENTVERSIONDATE               = 'July 16, 2018' ;
 ...
 {*)}
  ...
  const
    ...
    ContestsBooleanArray                : array[ContestType] of Byte =
      ...
      ({Name: 'RUSSIAN-160';                }ciCDC1 + ciCQZoneMode0 + ciVHFEnabled0 + ciErmak1 + ciQB0 + ciQM0 + ciMB0 + ciMM0),      // 4.65.
