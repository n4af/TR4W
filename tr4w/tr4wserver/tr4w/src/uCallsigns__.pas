...
//  destructor Destroy; override;
    ...
    function AddIniitialExchange(const Call, InitialExchangeString: CallString): boolean;
    function GetIniitialExchange(const Call: CallString): CallString;
    ...
    procedure Display(dBand: BandType; dMode: ModeType);
...
function TCallsignsList.GetIniitialExchange(const Call: CallString): CallString;
var
  Index                            : integer;
begin
...
function TCallsignsList.AddIniitialExchange(const Call, InitialExchangeString: CallString): boolean;
label
  Add;
var
  Index                            : integer;
...
function TCallsignsList.CompareStrings(const s1 {edx}, s2 {ecx}: CallString): integer;
begin
...
//      if TempMode = FM then TempMode := Phone;
      PossibleCallsList[Result].FCall := FList^[Index].FCall;
      PossibleCallsList[Result].FDupe :=
...
procedure TCallsignsList.Display(dBand: BandType; dMode: ModeType);
var
  TempDSHandle                     : HWND;
  P1                               : PChar;
  P2                               : PChar;
...
begin
  ...
  TempDSHandle := tr4w_WindowsArray[tw_DUPESHEETWINDOW_INDEX].WndHandle;
  ...
  Mode := dMode;
{
  ...
  if not ColumnDupeSheetEnable then
    SendDlgItemMessage(TempDSHandle, 101, LB_RESETCONTENT, 0, 0)
  else
    for Index := 48 to 57 do SendDlgItemMessage(TempDSHandle, Index, LB_RESETCONTENT, 0, 0);

  SendDlgItemMessage(TempDSHandle, 101, WM_SETREDRAW, wParam(False), 0);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SendDlgItemMessage(TempDSHandle, 101, LB_ADDSTRING, 0, integer(@FList^[Index].FCall[1]));
    end;
  end;
  ...
  wsprintf(wsprintfBuffer, TC_DUPESHEET);
  asm add esp,16
  end;
  Windows.SetWindowText(TempDSHandle, wsprintfBuffer);
end;
