...
procedure DestroyDlPortio;
begin
  if (DriverIsLoaded) then DriverDestroy;
end;
...
procedure NoDLPortioMessage;
begin;
  Format(wsprintfBuffer, TC_DLPORTIODRIVERISNOTINSTALLED + ': %s: %s', FLastError, SysErrorMessage(dwStatus));
  showwarning(wsprintfBuffer);
  showwarning('Download and install DlPortIO driver from'#13#10'http://www.tr4w.com/files/port95nt.exe');
  halt;
end;
